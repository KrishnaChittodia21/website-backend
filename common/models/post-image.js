...
module.exports = function(PostImage) {
    ...
    PostImage.app.models.ImageFile.upload(ctx.req, ctx.res, options,
      (err, file) => {
        if (err) {
          ...
          sharp('./server/storage/' + ctx.req.params.container +
          '/' + fileInfo.name)
            .resize(100)
            .toFile('./server/storage/' + ctx.req.params.container + '/100-' + fileInfo.name, (err) => {
              if (!err) {
                PostImage.create({
                  url: CONTAINER_URL + fileInfo.container +
                  '/download/' + fileInfo.name,
                  thumbnail: CONTAINER_URL + fileInfo.container +
  ...
  PostImage.remoteMethod(
    'upload', {
      ...
      returns: {
        args: 'fileObject', type: 'object', root: true,
      },
      http: {
        verb: 'post',
      },
